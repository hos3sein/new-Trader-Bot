...
while True:
    time.sleep(25)
    run_client('its the spider commingggggggggggggggggg')
    if (getTime()['minute'] == 19):
        logging.info('main=> time for checking the price...')
        run_client('main=> time for checking the price...')
        ...
        if (position.state == 0):               # waiting for coming the price to my zone
            ...
            run_client('main=> im waiting for coming the price to my zone...')
            ...
            run_client('main=> i am waiting for making the buttom or top level...')
            if ((cData['rsi'] > 70 and cData['price'] < position.maxTouch.price) or (cData['rsi']<30 and cData['price'] > position.minTouch.price)):
                ...
                run_client('main=> the top zone made in the market ....')
                ...
                if (cData['rsi'] > 70):
                    ...
                    run_client('main=> the max price updated to the upper price ....')
                    position.updatePrice(cData , 1)
                    time.sleep(60)
                elif(cData['rsi']<30):
                    logging.info('main=> the min price updated to the lower price ....')
                    run_client('main=> the min price updated to the lower price ....')
            ...
            run_client('main=> im waiting for making retouch the top or buttom ...')
            if (position.positionType == 0):
                if (cData['price'] in range(position.minTouch.price-(position.minTouch.price*0.005) , position.minTouch.price+(position.minTouch.price*0.005))):
                    ...
                    run_client('main=> wow! the market retouch the zone...')
                    ...
                    run_client('main=> shit the zone was not stable and break...')
                    position.changeStatus(1 , cData)
                    time.sleep(60)
            else:
            ...
            run_client('main=> im waiting for approv the divergance')
            if (position.positionType == 0):
                if (cData['price'] > position.minTouch.price):
                    if (position.divergance > position.minTouch.rsi):
                        ...
                        run_client('main=> the divergance happended....')
            ...
            run_client('main=> im waiting for back to the safeZone')
